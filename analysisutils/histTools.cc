...
void getHist( TH1F* &backgroundhist, std::string filename, std::string histname){
 TFile *infile = TFile::Open(filename.c_str(),"READ");
  if(infile->IsZombie())  cout<<"can not read from file :"<<filename<<endl;
  backgroundhist = dynamic_cast<TH1F*>(infile->Get(histname.c_str())->Clone("backgroundhist"));
  return;
}
